...
$result1 = mysqli_query($conn,"SELECT `issue`.`name`, `issue`.`title`, `issue`.`date`, `issue`.`ubin`, `issue`.`adm`, `issue`.`status`, `issue`.`batch` FROM issue,student where `student`.`view`= 0 and `student`.`adm` = `issue`.`adm` and `issue`.`status` = 'Not returned' order by `date` desc");
?>
<html lang="en">
<head>
...
<br><br>
 ...
 <form method="post" action="t_viewIssued2.php">
         ...
         <br>
       <a href="t_advSearch.php" class="btn btn-primary">Advanced Search</a>
...
<td>          <?php include't_filter.php'?></td>
                ...
                for (j = 0; j < filter.length; j++) {
                  ...
                  for (i = 0; i < tr.length; i++) {
                    ...
                    if (src!='' && td && td2 && td3  && td5 && td4) {
                     ...
                     txtValue4 = td5.textContent || td5.innerText;
                      txtValue5 = td4.textContent || td4.innerText;
