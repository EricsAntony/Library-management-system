...
if(isset($_SESSION["sess_user"])){
 include_once 'db.php';
 $batch = $_POST['batch'];
 $result1 = mysqli_query($conn,"SELECT `issue`.`name`, `issue`.`title`, `issue`.`date`, `issue`.`ubin`, `issue`.`adm`, `issue`.`status`, `issue`.`batch`, `issue`.`issue_by` FROM issue,student where `student`.`view`= 0 and `issue`.`batch`='$batch' and `student`.`adm` = `issue`.`adm` and `issue`.`status` = 'Not returned' order by `date` desc");
 ?>
 <html lang="en">
 <head>
          ...
          <div class="container-fluid">
            ...
            <div class="card shadow mb-4">
              ...
              <form method="post" action="viewIssued1.php">
                ...
                <a href="viewIssued.php" class="btn">Show all</a>
               </div>
             </form>
